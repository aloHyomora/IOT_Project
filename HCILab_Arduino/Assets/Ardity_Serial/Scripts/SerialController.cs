...
public class SerialController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        Debug.Log(message);
        string[] datas = IOBridge.TransformReceivedData(message);
        // -1 ~ 1 사이 값으로 변환
        int result1;
        int result2;
        if (int.TryParse(datas[0], out result1)) {
            float xValue = ((float)result1 / 1023) * 2 - 1;
            AstronautPlayer.AstronautPlayer.Instance.xValue = xValue;
        } else {
            // 유효하지 않은 입력 처리
            Debug.Log("Invalid input data");
        }
        if (int.TryParse(datas[1], out result2)) {
            float yValue = ((float)result2 / 1023) * 2 - 1;
            AstronautPlayer.AstronautPlayer.Instance.yValue = yValue;
        }
        else
        {
            // 유효하지 않은 입력 처리
